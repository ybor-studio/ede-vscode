...
export async function activate(context: vscode.ExtensionContext) {
  const logger = new Logger("Ybor Studio EDE");
  const provider = new TunnelProvider(logger, context);

  context.subscriptions.push(
    vscode.commands.registerCommand("ede-vscode.tunnel.logs", () =>
      logger.show()
    ),
    vscode.commands.registerCommand("ede-vscode.tunnel.restart", () =>
      provider.restart()
    ),
...
class TunnelProvider implements vscode.TunnelProvider {
  ...
  private async consentPublicPort(portNumber: number) {
    ...
    const r = await vscode.window.showWarningMessage(
      `You're about to create a publicly forwarded port. Anyone on the internet will be able to connect to the service listening on port ${portNumber}. You should only proceed if this service is secure and non-sensitive.`,
      { modal: true },
      continueOpt,
      dontShowAgain
    );
  ...
  private async setupPortForwardingProcess() {
    ...
    const child = spawn("/usr/bin/sleep", ["10"], {
      stdio: "pipe",
      env: {
        ...process.env,
        NO_COLOR: "1",
    ...
    vscode.window.withProgress(
      {
        ...
        title: `Starting port forwarding system ([Show Log](command:tunnel-forwarding.showLog))`,
      },
      () => progressP.p
    );
